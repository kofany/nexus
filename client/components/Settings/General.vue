		...
		<div v-if="!store.state.serverConfiguration?.public">
			...
			<label class="opt">
				...
				<input
					...
					name="awayMessage"
					class="input"
					placeholder="Away message if NexusIRC is not open"
				/>
			</label>
		</div>
	</div>
...
export default defineComponent({
	...
	setup() {
		...
		const registerProtocol = () => {
			...
			window.navigator.registerProtocolHandler("irc", uri, "NexusIRC");
			// @ts-expect-error
			window.navigator.registerProtocolHandler("ircs", uri, "NexusIRC");
		};
