...
export class IrssiClient {
	...
	private setupIrssiEventHandlers(): void {
		...
		this.irssiConnection.on("error", (msg: FeWebMessage) => {
			log.error(`User ${chalk.bold(this.name)}: irssi WebSocket error`);
		});
	...
	async handleInput(socketId: string, data: {target: number; text: string}): Promise<void> {
		log.info(`[DEBUG handleInput] socketId=${socketId}, target=${data.target}`);
	...
	private handleMessage(networkUuid: string, channelId: number, msg: Msg): void {
		log.debug(
			`[IrssiClient] Message received: type=${msg.type}, from=${msg.from?.nick || "unknown"}`
		);
		...
		if (this.messageDedup.isDuplicate(networkUuid, channelId, msgTime, msgText)) {
			log.debug(`[IrssiClient] Skipping duplicate message`);
			return;
		}
	...
	executeIrssiCommand(command: string, server?: string): void {
		...
		log.info(`[IrssiClient] Executed command${server ? ` on ${server}` : ""}`);
	}
