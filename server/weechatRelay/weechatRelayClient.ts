...
export class WeeChatRelayClient extends EventEmitter {
    ...
    private handleData(data: Buffer): void {
        ...
        while ((newlineIndex = this.buffer.indexOf("\n")) !== -1) {
            ...
            log.info(`${chalk.cyan("[WeeChat Relay Client]")} Received command line (${line.length} chars)`);
    ...
    private handleTextCommand(line: string): void {
        log.info(`${chalk.cyan("[WeeChat Relay Client]")} Parsing text command (${line.length} chars)`);
