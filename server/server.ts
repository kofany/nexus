...
// Basic IRC commands for autocomplete (replaces legacy inputs system)
const basicIrcCommands = [
...
(Node.js ${chalk.green(process.versions.node)} on ${chalk.green(process.platform)} ${
    ...
    server.listen(listenParams, () => {
        ...
        sockets.on("connect", (socket) => {
            ...
            const authHandler = (data: any) => {
                void performAuthentication
                    .call(socket, data)
                    .catch((err) => log.error("auth:perform error:", err));
            };
        ...
        const exitGracefully = async function () {
            ...
            // 2. Wait for all clients to quit gracefully (with timeout per client)
            if (manager) {
                const quitPromises = manager.clients.map((client) => {
                    return new Promise<void>((resolve) => {
                        ...
                        const timer = setTimeout(() => {
                            ...
                            finished = true;
                            log.warn(`Client ${client.name} quit timeout - continuing shutdown`);
                            resolve();
                        ...
                        client
                            ...
                            .catch((err) => {
                                ...
                                finished = true;
                                clearTimeout(timer);
                                log.error(`Error during client ${client.name} quit:`, err);
                                resolve();
                            });
                    });
                });
...
function initializeIrssiClient(
    ...
    const continueInit = (tokenToSend?: string) => {
        ...
        // Send commands list
        socket.emit("commands", basicIrcCommands);
...
async function performAuthentication(this: Socket, data: AuthPerformData) {
    ...
    const socket = this;
    let client: IrssiClient | undefined;
    let token: string;
