    ...
    public getHistoryFormatted(): string {
        return this.history
            .map((entry, idx) => {
                ...
                return `[${idx}] ${timestamp}\n  Old: ${JSON.stringify(entry.oldValue)}\n  New: ${JSON.stringify(entry.newValue)}${entry.stack ? `\n  Stack: ${entry.stack}` : ""}`;
            })
            .join("\n\n");
    }
...
${this.history.slice(-5).map((h, i) => `  ${i}: "${JSON.stringify(h.oldValue)}" → "${JSON.stringify(h.newValue)}"`).join("\n")}
        `.trim();
    }
}
