...
function copyStaticFiles() {
	...
	// FontAwesome fonts are now handled by Vite directly via CSS imports
}
	...
	plugins: [
		...
		}),
		{
			...
			closeBundle() {
				...
				// Generate CSS source map reference for compatibility with tests
				...
				if (fs.existsSync(cssPath)) {
					...
					if (!css.includes("sourceMappingURL")) {
						...
						// Create a basic source map file
						const sourceMap = {
							version: 3,
							sources: ["../client/css/style.css"],
							names: [],
							mappings: "",
							file: "style.css",
						};
						fs.writeFileSync(
							path.resolve(__dirname, "public/css/style.css.map"),
